  ...
  Future<List<BookingDetail>> getBookingDetails() async {
    try {
      final String jsonString = await rootBundle.loadString(
        'assets/data/mock_booking.json',
      );
      print('JSON Content: $jsonString');
      final Map<String, dynamic> jsonData = jsonDecode(jsonString);

      final List<dynamic> bookingDetails =
          jsonData['CozyCare.BookingDb']?['BookingDetails'] ?? [];
      final List<dynamic> services =
          jsonData['CozyCare.CatalogDb']?['Services'] ?? [];
      final List<dynamic> taskClaims =
          jsonData['CozyCare.JobDb']?['TaskClaims'] ?? [];
      ...
      final availableDetails =
          bookingDetails
              .where((e) => !claimedDetailIds.contains(e['detailId']))
              .toList();
      print('Available Details: $availableDetails');

      return availableDetails
          .map(
            (e) => BookingDetail.fromMap({
              ...e,
